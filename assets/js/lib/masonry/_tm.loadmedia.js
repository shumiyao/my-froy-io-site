...
var loadMedia = (function () {
  ...
  function LoadMedia(element, options) {
    ...
    try {
      ...
      console.log(
        `${error} - format must be: let x = new loadMedia('.selector' or NodeList,{options})`
      ); // eslint-disable-line
    }
    ...
    const processMedia = (media, mediaType) => {
      ...
      switch (mediaType) {
        case "image":
          ...
          proxyImage.src = image.dataset.src.replace(/&amp;/g, "&");
          ...
          if ("decode" in proxyImage) {
            proxyImage
              ...
              .catch((encodingError) => {
                console.log(encodingError); // eslint-disable-line
                onError(proxyImage);
              });
        ...
        case "video":
          ...
          video.querySelectorAll("source").forEach(function (source) {
            if (
              ...
              source.src = source.dataset.src.replace(/&amp;/g, "&");
              video.load();
            }
          });
    ...
    const onSuccess = (media, refItem) => {
      ...
      if (media.tagName == "IMG" && !plugin.settings.backgroundImage)
        addLoadedMedia(media, refItem);
    ...
    const setBgImage = (media, refItem) => {
      ...
      refItem.style.cssText =
        refItem.style.cssText + `background-image:url(${media.src})`;
      classList(refItem)
        .addClass("bg-image")
        .addClass("bg-cover")
        .addClass("bg-center");
    ...
    const onError = (event) => {
      ...
      // let errorMessage = `The following image: ${target.src} did not load.`; // eslint-disable-line
    ...
    plugin.initialize = () => {
      ...
      let elementDataSrc = !plugin.elements.dataset
        ? false
        : plugin.elements.dataset.src
        ? true
        : false;
