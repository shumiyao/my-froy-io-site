...
var inView = (function () {
  ...
  function InView(element, options) {
    ...
    try {
      ...
      console.log(
        `${error} - format must be: let x = new tmInView('.selector' or NodeList,{options})`
      );
    }
    ...
    const setTimeoutData = (element, value) =>
      element.setAttribute(timeOutData, value);
    ...
    const delayCallbackInView = (element, elementObserver) => {
      ...
      timeoutId = setTimeout(function () {
        ...
        if (iObserve && plugin.settings.unObserveViewed)
    ...
    let observeOnScroll = () => {
      ...
      if (debounceTimeout) {
        document.cancelAnimationFrame(debounceTimeout);
      }
      ...
      debounceTimeout = document.requestAnimationFrame(function () {
        // Check element state
        plugin.elements.forEach(function (element) {
          ...
          let callbackInFlag =
            ...
            !plugin.settings.loopCallbackOnScroll
              ? true
              : false;
          let callbackOutFlag =
            element.classList.contains("out-of-view") &&
            !plugin.settings.loopCallbackOnScroll
              ? true
              : false;
          ...
          if (
            elementArray.length === plugin.elements.length &&
            plugin.settings.unObserveViewed
          )
            document.removeEventListener("scroll", observeOnScroll, false);
    ...
    const isElementVisible = (element) => {
      ...
      let windowHeight =
        document.innerHeight + plugin.settings.detectionBuffer ||
        document.documentElement.clientHeight + plugin.settings.detectionBuffer;
      let threshold = element.getAttribute("data-threshold")
        ? element.getAttribute("data-threshold")
        : plugin.settings.threshold;
      // eslint-disable-next-line
      let isBeyondRect =
        Math.floor(
    ...
    plugin.initialize = () => {
      ...
      // Check element
      plugin.elements = NodeList.prototype.isPrototypeOf(plugin.elements)
        ? plugin.elements
        : document.querySelectorAll(plugin.elements); // eslint-disable-line
      ...
      if (iObserve) {
        ...
        document.addEventListener("scroll", observeOnScroll, false);
