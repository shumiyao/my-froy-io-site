  ...
  */
  function ResponsiveVideo(element, options) {
    ...
    try {
      ...
      console.log(
        `${error} - format must be: let x = new tmResponsiveVideo('.selector' or NodeList,{options})`
      );
    }
    ...
    const addWrapper = (video) => {
      if (!video.parentNode.classList.contains("video-container")) {
        ...
        if (!video.hasAttribute("data-bg-video"))
          classList(newWrapper).addClass("video-container");
    ...
    const calcPadding = (video) => {
      return (
        (parseInt(video.height, 10) / parseInt(video.width, 10)) * 100 + "%"
      );
    };
    ...
    const resizeVideo = () => {
      document.querySelectorAll(plugin.elements).forEach(function (video) {
        if (video.hasAttribute("data-bg-video")) {
          ...
          let parentRatio =
            parentW > parentH
              ? calcRatios(parentW, parentH)
              : calcRatios(parentH, parentW);
          newVideoH =
            parentRatio >= videoRatio ? parentH * parentRatio : parentH;
          newVideoW = newVideoH * videoRatio;
          video.style.width = newVideoW + "px";
          video.style.height = newVideoH + "px";
          video.style.top =
            parentRatio >= videoRatio
              ? (parentH - newVideoH) / 2 + "px"
              : (newVideoH - parentH) / 2 + "px";
    ...
    plugin.initialize = () => {
      ...
      document.querySelectorAll(plugin.elements).forEach(function (video) {
        ...
        let bgVideo = video.hasAttribute("data-bg-video")
          ? true
          : plugin.settings.bgVideo;
        addWrapper(video);
        if (mediaType && !bgVideo) {
          makeResponsive(video);
        } else if (bgVideo) {
          document.addEventListener("resize", resizeVideo, false);
    ...
    plugin.destroy = () => {
      ...
      if (document.querySelector("[data-bg-video]"))
        document.removeEventListener("resize", resizeVideo, false);
